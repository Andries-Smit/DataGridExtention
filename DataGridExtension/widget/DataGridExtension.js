...
require(["dojo/json", "dojo/dnd/Moveable", "dojo/_base/declare", "dojo/_base/event", "dojo/dnd/Mover", "dojo/dom-geometry"], function (JSON, Moveable, declare, event, Mover, domGeom) {
    ...
    var widget = {
        ...
        showError: function(msg) {
            console.error(msg);
            this.domNode.appendChild( mxui.dom.create("div", {style:"color:red"}, msg));
        },
        ...
        setupInlineColumns: function () {
            if (this.inlineButtons.length > 0) {
                ...
                var button = new mxui.widget.Button({
                            caption: self.inlineButtons[0].caption,
                            iconUrl: self.inlineButtons[0].icon,
                            onClick: function(e){alert(1);},
                            //onClick: dojo.hitch(this, this.onclickEventInline, self.inlineButtons[0].onClickMicroflow),
                            renderType: self.inlineButtons[0].buttonStyle === "Link" ? "link" : "button",
                            cssclass: self.inlineButtons[0].buttonStyle
                        });
                ...
                dojo.aspect.around(this.grid, "_gridbodyFillRow", function (originalMethod) {
                    ...
                    return function (mxobj, gridMatrixRow, gridAttributes) {
                        ...
                        //self.grid.connect(button.domNode, "onclick",function (e) {
                           // alert(3);
                           //         });
                    };
                });
                
                /*dojo.aspect.around(this.grid, "gridbodyFillCell", function (originalMethod) {
                    // wrap arround the grid function to change stuff before and after.
                    return function (attrs, mxobj, attrname) {
                        var content = originalMethod.apply(this, arguments);
                        var button = new mxui.widget._Button({
                            caption: self.inlineButtons[0].caption,
                            iconUrl: self.inlineButtons[0].icon,
                            onClick: dojo.hitch(this, this.onclickEventInline, self.inlineButtons[0].onClickMicroflow),
                            renderType: self.inlineButtons[0].buttonStyle === "Link" ? "link" : "button",
                        ...
                        return button.domNode.outerHTML;
                
                    };
                });*/
            }
        },
        onclickEventInline: function(microflow, evt){
              if (microflow !== "") {
                mx.data.action({
                    error: function () {
